...
modal.addEventListener("click", function (event) {
  if (event.target === addShortcut) {
    ...
    if (name && url) {
      ...
      newShortcut.setAttribute("href", url);
      const domain = url;
      const size = 64;
...
document.getElementById("edit-context-menu-option").addEventListener("click", () => {
  contextMenu.style.display = "none";

  const name = selectedShortcut.querySelector("p").textContent;
  const url = selectedShortcut.getAttribute("href");
  ...
  document.getElementById("edit-shortcut").addEventListener("click", () => {
    ...
    selectedShortcut.setAttribute("href", newURL); 
