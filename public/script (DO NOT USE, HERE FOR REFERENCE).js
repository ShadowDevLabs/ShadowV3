...
const decode = (i) => {return self.__uv$config.decodeUrl(i)}
...
function getSrc(i = currentTabIndex) {
  let iframe = iframesContainer.children[i].contentDocument;
  const src = "about:blank" != iframe.location.href ? href : iframe.src;
  src = decode(src.replace("uv/service/", ""));
  alert(src)
  return src;
}

function test(index = currentTabIndex) {
  let tab = tabs[index];
  iframe = document.getElementById("iframes-container").children[index];
  const title = iframe.contentDocument.title;
  tab.getElementById("title").textContent = title;
  let src = getSrc();
  tab.img.src = `https://www.google.com/s2/favicons?domain=${src}&sz=64`;
}
...
function switchTab(index) {
  ...
  let currentSrc = iframesContainer.children[index].contentDocument.location.href;
  console.log("Before: "+currentSrc)
  if (currentSrc=="about:blank" || !currentSrc.includes("/uv/service/")) {
    currentSrc = iframesContainer.children[index].src
    currentSrc = "shadow://".concat(currentSrc.replace(location.href,"").replace(".html","").replace("pages/",""));
  } else {
    currentSrc = decode(currentSrc.replace(location.href, "").replace("uv/service/", ""));
  }
  console.log("After: "+currentSrc)
...
function exit() {
if (window.confirm("Are you sure you want to close this tab?")) { window.close() }
}
